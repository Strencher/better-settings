...
function getSetting(id, defaultValue) {
  if (!id.includes("."))
    return SettingsManager.getSetting(id, defaultValue);
...
function setSetting(id, newValue) {
  if (!id.includes("."))
    return SettingsManager.updateSetting(id, newValue), emitChange(id);
  const items = id.split(".");
  const first = items.shift();
...
// #region react
var React = require('powercord/webpack').React;
// #endregion react
...
// #region @structs
const PCPlugin = require("powercord/entities").Plugin;
class Plugin extends PCPlugin {
  _shouldUnregisterSettings = false;
  onStart() {
  }
  onStop() {
  }
